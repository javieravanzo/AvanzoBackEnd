  ...
  if (email !== "" && password !== "") {
    try {
      ...
      if (result.status === 200) {
        ...
        const loginhistory = {
          lohi_ip: ip,
          lohi_browser: browser,
          lohi_so: so,
          idUser: result.data.user_info.idUser

        };
        //Registramos en el historico del logueo
        const historyRegistered = await dbSequelize.loginhistory.create(loginhistory);

        if (historyRegistered !== null) {
          res.status(result.status).json(result.data);
        }else{
          res.status(400).json({message:"Error registrando el historico de logueo"});
        }
